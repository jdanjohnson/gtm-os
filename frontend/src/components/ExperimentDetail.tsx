          ...
          <div key={phase} className="flex items-center gap-1.5">
            <div
              className={clsx(
                ...
                isActive && !isPaused && "ring-1 ring-black/5",
                isComplete && "bg-emerald-500/10 text-emerald-600 border-emerald-500/20",
                !isActive && !isComplete && "text-gray-400 border-transparent",
                isPaused && isActive && "phase-paused ring-1 ring-black/5",
              )}
            >
            ...
            {i < PHASES.length - 1 && (
              <div
                className={clsx(
                  ...
                  isComplete ? "bg-emerald-500/40" : "bg-gray-200",
                )}
              />
            )}
          </div>
    ...
    return (
      <div className="flex h-full w-full flex-col items-center justify-center text-center">
        <div className="glass-card p-8">
          ...
          <p className="text-sm text-gray-500">Select an experiment from the sidebar.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex h-full w-full flex-col overflow-y-auto p-6 animate-fade-in">
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          {experiment.hypothesis && (
            <p className="mt-1 text-sm text-gray-500">{experiment.hypothesis}</p>
          )}
          <div className="mt-1 text-[10px] text-gray-400 font-mono">id: {experiment.id}</div>
        </div>
        <button
          onClick={onClear}
          className="glass-pill flex items-center gap-1 text-gray-400 hover:text-gray-700 transition-colors"
        >
          <X size={12} /> close
        </button>
      </div>
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        ...
        <button
          ...
          className="glass-pill flex items-center gap-1.5 text-gray-400 hover:text-gray-700 transition-colors"
        >
          <Calendar size={12} />
      ...
      <div className="space-y-2">
        ...
        {runs.length === 0 && (
          <p className="text-xs text-gray-500">No runs yet. Click "Run Tick" to start.</p>
        )}
        {runs.map((r) => (
          <div key={r.id} className="glass-card !rounded-xl overflow-hidden">
            ...
            >
              <div className="flex items-center gap-2">
                {r.status === "completed" && <CheckCircle2 size={14} className="text-emerald-600" />}
                ...
                <span className="text-sm text-gray-900">{r.phase}</span>
              ...
              <div className="flex items-center gap-3">
                ...
                {expandedRun === r.id ? <ChevronDown size={12} className="text-gray-400" /> : <ChevronRight size={12} className="text-gray-400" />}
              </div>
            </button>
            {expandedRun === r.id && (
              <div className="border-t border-black/[0.06] px-4 py-3 text-xs animate-fade-in">
                {r.error && (
                  <div className="mb-2 rounded-lg bg-rose-500/10 border border-rose-500/20 p-2 text-rose-600">
                    {r.error}
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {r.tools_used.map((t, i) => (
                        <span key={i} className="glass-pill text-gray-500">{t.name}</span>
                      ))}
                    </div>
                  </div>
                )}
